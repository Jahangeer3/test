...
class Home extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      responder: firebase.auth().currentUser.email,
  ...
  render() {
    ...
    const result = Object.keys(this.state.markerList).map(
      (key) => this.state.markerList[key],
    );
          ...
          <View style={{flex: 1}}>
            <MapboxGL.MapView
              ...
              <MapboxGL.Camera
                ...
                animationDuration={0}
                ref={(c) => (this.camera = c)}></MapboxGL.Camera>
